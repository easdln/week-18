...
window.addEventListener('load', function(){
  let element = '';
  for(let obj of arr) {
      element += `<div class="flex-elem">
          <h2>${obj.name}</h2>
          <p>Вселенная: ${obj.universe}</p>
          <p>Альтер эго: ${obj.alterego}</p>
          <p>Род деятельности: ${obj.occupation}</p>
          <p>Друзья: ${obj.friends}</p>
          <p>Навыки: ${obj.superpowers}</p>
          <img src="${obj.url}"/>
...
const createRating = () =>{
  ...
  const starsArr = ['star_5','star_4','star_3','star_2','star_1'];
  ratingContainer.forEach((container,index) =>{
    container.innerHTML = starsArr.map((star,ind) => 
      `<input type="radio" id="radio_${index}_${star}" name="rating${index}" value="${5-ind}"/>
        <label for="radio_${index}_${star}"></label>`).join('')
      })
}

const ratingState = {};
const setRadioinputs = () => {
  const radioInputs = document.querySelectorAll('input')
  radioInputs.forEach((input)=>{
    ratingState[input.id] = input.checked;
  })
  localStorage.setItem('radioInputs',JSON.stringify(ratingState))
}

container.addEventListener('change', setRadioinputs)

const getRadioInputs = () =>{
  const radioInputsState = JSON.parse(localStorage.getItem('radioInputs'))
  for(let inputId in radioInputsState){
    const radio = document.getElementById(inputId);
    if(radio){
      radio.checked = radioInputsState[inputId]
    }
  }
}
